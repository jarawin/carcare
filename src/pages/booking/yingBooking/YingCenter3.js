    ...
    <div className="flex lg:flex-row mt-16">
      {dataCard.map((item, idx) => {
        return (
          <div
            key={idx}
            className="mx-auto bg-red-200 my-14 w-56 h-96 rounded-3xl mb-10 shadow-gray-500 shadow-2xl"
          >
            <img
              className="rounded-t-3xl h-40 w-full object-cover object-center"
              src={item.image}
            ></img>

            <h1 class=" pl-5 pr-5 py-3 text-2xl text-center font-bold text-red-600 leading-tight ">
              {item.title}
              <br />
